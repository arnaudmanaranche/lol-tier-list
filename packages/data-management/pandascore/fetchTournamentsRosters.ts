...
export async function fetchPandascoreTournamentsRosters(tournamentIds: string[]) {
  ...
  const fetchRostersPromise = tournamentIds.map(async (tournamentId) => {
    ...
    if (response.status === 200) {
      ...
      throw new Error(`Cannot fetch PandaScore rosters for tournament ID: ${tournamentId}`)
    }
  })
